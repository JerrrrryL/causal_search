...
from sketch_search import JoinSketch, DataMarket, cleanup, SemiRing
...
def differential_entropy(kde, bounds):
    """Estimate differential entropy given a KDE object and bounds."""
    if len(bounds) == 2:  # Single variable
        return -quad(lambda x: kde(np.array([[x]]))[0] * np.log(kde(np.array([[x]]))[0]), bounds[0], bounds[1])[0]
    elif len(bounds) == 4:  # Joint distribution
        return -dblquad(lambda x, y: kde(np.array([[x], [y]])) * np.log(kde(np.array([[x], [y]]))),
                        bounds[0], bounds[1], lambda x: bounds[2], lambda x: bounds[3])[0]

def estimate_mutual_information(df, col1, col2):
    """Estimate mutual information between two continuous variables from a DataFrame."""
    x = df[col1].values
    y = df[col2].values
    ...
    h_x = differential_entropy(kde_x, bounds_x)
    h_y = differential_entropy(kde_y, bounds_y)
    h_xy = differential_entropy(kde_xy, (*bounds_x, *bounds_y))
...
def rbf_kernel(X, sigma=None):
    ...
    if sigma is None:
        sigma = np.std(X)  # Heuristic based on data std
...
def center_kernel_matrix(K):
    ...
    return np.dot(H, np.dot(K, H))

def hsic_test(df, col_x, col_y, sigma_x=None, sigma_y=None):
    """Compute the HSIC statistic for two columns of a DataFrame."""
    X = df[[col_x]].to_numpy()
    Y = df[[col_y]].to_numpy()
    
    # Compute the RBF kernel matrices for X and Y
    Kx = rbf_kernel(X, sigma=sigma_x)
    Ky = rbf_kernel(Y, sigma=sigma_y)
    
    # Center the kernel matrices
    Hx = center_kernel_matrix(Kx)
    Hy = center_kernel_matrix(Ky)
    
    # Compute the HSIC statistic
    hsic_stat = np.trace(np.dot(Hx, Hy)) / (len(X) - 1) ** 2
...
class ConDiscovery:
    ...
    def _align_treat_out_vec(
        ...
        self.cur_data_in = merged.set_index('index').loc[:, self.cur_data_in.columns]
    ...
    def bivariate_causal_discovery(self, t_sketch, seller_sketch, join_key, mfactor=1):
        ...
        if self.factorized_hist:
            ...
            def _outlier_inds(v):
                q1 = torch.quantile(v, 0.25)
                q3 = torch.quantile(v, 0.75)
                upper_bound = (q3 + 1.5*q3 - q1).to(self.device)
                mask = torch.where(
                    (v <= upper_bound))[0].to(self.device)
                return mask
...
# ''')
            ...
            # print(f"joint hist shape: {joint_hist.shape}")
            # print(agg_c)
...
# ''')
    ...
    def preprocess_compute_te(self, df, join_keys, treatment, outcome, top_k=5):
        ...
        for join_key in join_keys:
            ...
            if tuple(join_key) in self.dm.seller_join_sketches:
                treat_out_instance = JoinSketch(join_key_domain=cur_domain, is_buyer=True, device=self.device)
        ...
        self.treat_out_sketches_cp = copy.deepcopy(self.treat_out_sketches)
    ...
    def compute_treatment_effect(self, df, join_keys, treatment, outcome, conf_type=None, search_iters=None):
        ...
        return causal_effect(self.cur_data_in, treatment, outcome)
...
#         def test_pruning(t_sketch, s_sketch, batch_id, join_key, k=5):
#             # get the source of the vectors within a local sketch cluster + treatment vector
#             # there is only constant vectors in this step
#             def get_conf_score(conf_inds, conf_vecs, score_vec):
#                 # The score of a confounder is the sum of mutual information using it as the cause to predict all other variables
#                 for i in range(len(conf_inds)-1):
#                     exp_sketch, targ_sketch = {}, {}
#                     for key, val in s_sketch.items():
#                         fil_sketch = val[:, conf_inds]
#                         exp_sketch[key] = fil_sketch[:, i+1:]
#                         targ_sketch[key] = fil_sketch[:, i:i+1]
                        
#                     exp_vecs = conf_vecs[:, i+1:]
#                     targ_vec = conf_vecs[:, i:i+1]
#                     # Here the normalization can be reused
#                     # mi: the i^th confounder as the cause and the remaining confounders as outcomes
#                     # mi_r: the remaining confounders as the cause and the i^th confounder as outcome
                    
#                     # TODO: This is the special case of bivariate causal discovery over a single sketch
#                     # However, need to consider over the join
#                     _, mi, mi_r = self.bivariate_causal_discovery(
#                         targ_sketch, exp_sketch, join_key)
#                     for j in range(i+1, len(conf_inds)):
#                         score_vec[i] += mi[j-i-1]**2
#                         score_vec[j] += mi_r[j-i-1]**2
            
#             mi_diffs, mi, mi_r = self.bivariate_causal_discovery(
#                 t_sketch, s_sketch, join_key
#             )
#             topk_mi_diffs, topk_mi_diff_inds = torch.topk(mi_diffs, k)
#             topk_mi, topk_mi_r = mi[topk_mi_diff_inds], mi_r[topk_mi_diff_inds]
            
#             if k is None:
#                 mi_diff_inds = torch.where(mi_diffs > self.mi_threshold)[0]
#                 mi_diff_sgf = mi_diffs[mi_diff_inds]
#                 return mi_diff_inds, mi_diff_sgf
#             else:
#                 topk_mi_diffs, topk_mi_diff_inds = torch.topk(mi_diffs, k)
#                 return topk_mi_diff_inds, topk_mi_diffs

#             topk_mi_diffs, topk_mi_diff_inds = torch.topk(mi_diffs, k)
#             topk_mi, topk_mi_r = mi[topk_mi_diff_inds], mi_r[topk_mi_diff_inds]
#             topk_conf_vecs = seller_exp[:, topk_mi_diff_inds]
...
#                     batch_id, inds=topk_mi_diff_inds
#                 ), 
#                 topk_conf_vecs
#             )
...
#                 return -1, -1, None
#             else:
#                 conf_inds = topk_mi_diff_inds[condition]
#                 conf_vecs = topk_conf_vecs[:, condition]
#                 score_vec = torch.zeros(len(condition) + 1).to(self.device)
            
#                 for ind, val in enumerate(conf_inds):
#                     # use last entry of the array to represent the score of treatment
#                     score_vec[-1] += topk_mi[condition][ind]**2
#                     score_vec[ind] += topk_mi_r[condition][ind]**2
#                 get_conf_score(conf_inds, conf_vecs, score_vec)

#                 if self.verbose:
#                     print("="*50)
#                     for ind, score in enumerate(score_vec):
#                         if ind == len(score_vec)-1:
#                             print(f'''
# Score of treatment is {score}''')
#                         else:
#                             conf = self.dm.seller_join_sketches[
#                                 join_key].get_seller_by_feature_index(batch_id, conf_inds[ind])
#                             print(f'''
# Dataset is {self.dm.seller_datasets[conf[0]][1]},
# Confounder is {conf[1]},
# Score is {score}''')
#                 min_score_ind = torch.argmin(score_vec)
#                 if min_score_ind == len(score_vec)-1: return -1, -1, None
#                 else: return conf_inds[min_score_ind], score_vec[
#                     min_score_ind], conf_vecs[:, min_score_ind]
        ...
        for join_key, treat_out_sketch in self.treat_out_sketches.items():
            ...
            if join_key not in self.dm.seller_join_sketches: continue
                ...
                for batch_id in range(search_sketch.sketch_loader.num_batches):
                    ...
                    # cur_ind, cur_score, cur_conf_vec = test_pruning(t_sketch, s_sketch, batch_id, join_key)
...
''')
    ...
    def _update_corpus_to_res(self, seller_id, conf_feature, conf_join_key, conf_batch_id, ind, conf_vec):
        ...
        if self.debug:
            print(f"Feature is {conf_feature}, join key is {list(conf_join_key)}")
        ...
        for join_key in self.treat_out_sketches.keys():
            ...
            if join_key == conf_join_key:
                ...
                for col in join_key:
                    cur_domain[col] = self.dm.join_key_domains[col]
                conf_update_sketch = JoinSketch(join_key_domain=cur_domain)
    ...
    def prune_confounders(self, buyer_df, dm, k):
        # def _get_res_t_o_sketch(join_key, conf_sketch, conf_batch_id, inds):
        #     t_o_sketch = self.treat_out_sketches[join_key].sketch_loader.get_sketches(0)
        #     t_sketch = self.treat_out_sketches[join_key].sketch_loader.get_sketches(0, [0])
        #     o_sketch = self.treat_out_sketches[join_key].sketch_loader.get_sketches(0, [1])
        #     c_sketch = conf_sketch.sketch_loader.get_sketches(conf_batch_id)
        #     t_o_slope, t_o_intercept = self._get_slope_intercept(
        #         t_o_sketch, c_sketch
        #     )
        #     cross_t_o = self._get_cross_term_residual(
        #         t_o_slope[:1], 
        #         t_o_intercept[:1], 
        #         t_o_slope[1:], 
        #         t_o_intercept[1:], 
        #         self.cross_t_o[join_key],
        #         t_sketch,
        #         o_sketch, 
        #         c_sketch
        #     )
        #     res_sketch = _get_residual(
        #         t_o_slope, 
        #         t_o_intercept, 
        #         t_o_sketch, 
        #         c_sketch
        #     )
        #     self.treat_out_sketches[join_key].sketch_loader.update_sketch(res_sketch, 0)
        #     self.cross_t_o[join_key] = cross_t_o
...
#             for join_key, batch_inds in seller_feature_inds.items():
#                 search_sketch = dm.seller_join_sketches[join_key]
#                 t_sketch = self.treat_out_sketches_cp[join_key].sketch_loader.get_sketches(0, [0])
#                 o_sketch = self.treat_out_sketches_cp[join_key].sketch_loader.get_sketches(0, [1])
#                 t_o = self.cross_t_o[join_key]
#                 for batch_id in batch_inds.keys():
#                     inds = torch.tensor(
#                         seller_feature_inds[join_key][batch_id]).to(self.device)
#                     s_sketch = search_sketch.sketch_loader.get_sketches(
#                         batch_id, inds
#                     )

#                     c_to, s_t, Q_t = t_sketch.c, t_sketch.s, t_sketch.Q
#                     s_o, Q_o = o_sketch.s, o_sketch.Q
#                     s_z, Q_z = s_sketch.s, s_sketch.Q

#                     c = torch.sum(c_to, dim=0)
#                     z = torch.sum(c_to * s_z, dim=0)
#                     z_z = torch.sum(c_to * Q_z, dim=0)
#                     t_t = torch.sum(Q_t, dim=0)
#                     t = torch.sum(s_t, dim=0)
#                     o = torch.sum(s_o, dim=0)
#                     o_o = torch.sum(Q_o, dim=0)
#                     t_z = torch.sum(s_t * s_z, dim=0)
#                     o_z = torch.sum(s_o * s_z, dim=0)
#                     t_o = torch.sum(self.cross_t_o[join_key], dim=0)
