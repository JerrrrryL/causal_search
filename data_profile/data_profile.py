...
class DataProfile:
    ...
    def generate_G(self, n, density=0.2):
        G = nx.DiGraph()
    ...
    def save_diagram(self, name, coeffs=None):
        plt.figure(figsize=(12, 8))
        pos = nx.kamada_kawai_layout(self.G, dist=None, weight='weight', scale=2)
        nx.draw(self.G, pos, with_labels=True, arrows=True, node_size=700, font_size=10)
    ...
    def generate_D_from_G(self, num_samples=1000, join_key=('join_key', 10000)):
        ...
        for node in self.ordered_nodes:
            ...
            if parents:
                node_coeffs = np.random.uniform(1, 2, size=len(parents))
                noise = np.random.uniform(0, 1, size=num_samples)
                self.noises[node] = noise
                D[node] = np.round(2*np.dot(D[parents], node_coeffs) / len(parents) + noise, 2)
                for i, parent in enumerate(parents):
                    edge_coeffs[(parent, node)] = 2*node_coeffs[i] / len(parents)
            else:
                noise = np.random.uniform(0, 1, size=num_samples)
                self.noises[node] = noise
